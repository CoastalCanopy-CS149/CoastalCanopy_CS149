...
def submit_report():
    ...
    try:
        detection_result, result_image_url, detected_class_names = detect(img , report_id)
    except MemoryError:
    ...
    if result.acknowledged:
        if "Mangrove_Destruction" in detected_class_names:
            send_email(report)
...
def detect(image, report_id):
    ...
    return output, response["secure_url"], detected_class_names
